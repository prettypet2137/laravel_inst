...
class EventReminderDayBefore extends Command
{
    /**
     ...
     * @var string
     */
    protected $name = 'events:event-reminder-day-before';
     ...
     * @var string
     */
    protected $description = 'Command for email reminder to be sent guests before day of event.';
    ...
    public function eventReminderDayBeforeToMainGuest($event, $guest) {
    ...
    public function eventReminderDayBeforeToSubGuest($event, $guest, $sub_guest) {
    ...
    public function handle()
    {
        ...
        $type = SmsReminderTypes::where(["type" => "Event Reminder Day Before"])->first();
        $this->sms_template = SmsTemplate::where(["reminder_id" => $type->id, "type" => "default"])->first();
        $type = EmailReminderTypes::where(["type" => "Event Reminder Day Before"])->first();
        $this->email_template = EmailTemplate::where(["reminder_id" => $type->id, "type" => "default"])->first();

        $dayBefore = date("Y-m-d", strtotime("+1 day"));
        Event::with("guests.sub_guests")->whereDate("start_date", "=", $dayBefore)->chunk(100, function ($events) {
            foreach ($events as $event) {
                if (count($event->guests)) {
                    foreach ($event->guests as $guest) {
                        $this->eventReminderDayBeforeToMainGuest($event, $guest);
                        if (count($guest->sub_guests)) {
                            foreach ($guest->sub_guests as $sub_guest) {
                                $this->eventReminderDayBeforeToSubGuest($event, $guest, $sub_guest);
                            }
                        }
                    }
                }
