...
class EventReminderDayOf extends Command
{
    /**
     ...
     * @var string
     */
    protected $name = 'events:event-reminder-day-of';
     ...
     * @var string
     */
    protected $description = 'Command for email reminder to be sent guests the day of event.';
    ...
    public function eventReminderDayOfToMainGuest($event, $guest) {
    ...
    public function eventReminderDayOfToSubGuest($event, $guest, $sub_guest) {
    ...
    public function handle()
    {
        ...
        $type = SmsReminderTypes::where(["type" => "Event Reminder Day Of"])->first();
        $this->sms_template = SmsTemplate::where(["reminder_id" => $type->id, "type" => "default"])->first();
        $type = EmailReminderTypes::where(["type" => "Event Reminder Day Of"])->first();
        $this->email_template = EmailTemplate::where(["reminder_id" => $type->id, "type" => "default"])->first();

        $day = date("Y-m-d");
        Event::with("guests.sub_guests")->whereDate("start_date", "=", $day)->whereTime("start_date", ">", "06:00")->chunk(100, function ($events) {
            foreach ($events as $event) {
                if (count($event->guests)) {
                    foreach ($event->guests as $guest) {
                        $this->eventReminderDayOfToMainGuest($event, $guest);
                        if (count($guest->sub_guests)) {
                            foreach ($guest->sub_guests as $sub_guest) {
                                $this->eventReminderDayOfToSubGuest($event, $guest, $sub_guest);
                            }
                        }
                    }
                }
