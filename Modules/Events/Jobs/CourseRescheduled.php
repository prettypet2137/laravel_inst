...
class CourseRescheduled implements ShouldQueue
{
    ...
    public function courseRescheduledToMainGuest($guest) {
    ...
    public function courseRescheduledToSubGuest($guest, $sub_guest) {
    ...
    {
        $this->event = Event::find($this->guest->event_id);
        $this->sms_account = SmsAccount::first();
        $type = SmsReminderTypes::where(["type" => "Course Rescheduled"])->first();
        Log::info($this->event);
        ...
        $type = EmailReminderTypes::where(["type" => "Course Rescheduled"])->first();
        ...
        $this->courseRescheduledToMainGuest($this->guest);
        foreach ($this->guest->sub_guests as $sub_guest) {
            $this->courseRescheduledToSubGuest($this->guest, $sub_guest);
        }
    }
}
