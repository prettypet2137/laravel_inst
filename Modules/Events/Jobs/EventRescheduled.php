...
class EventRescheduled implements ShouldQueue
{
    ...
    public function eventRescheduledToMainGuest($guest) {
    ...
    public function eventRescheduledToSubGuest($guest, $sub_guest) {
    ...
    public function handle()
    {
        ...
        $type = SmsReminderTypes::where(["type" => "Event Rescheduled"])->first();
        ...
        $type = EmailReminderTypes::where(["type" => "Event Rescheduled"])->first();
        ...
        foreach ($this->event->guests as $guest) {
            $this->eventRescheduledToMainGuest($guest);
            foreach ($guest->sub_guests as $sub_guest) {
                $this->eventRescheduledToSubGuest($guest, $sub_guest);
            }
        }
    }
}
